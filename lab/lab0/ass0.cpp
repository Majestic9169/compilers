...
public:
  ...
  std::string access(std::string var) {
    if (map.find(var) != map.end())
      ...
      std::cerr << "\n\033[31m*** Undefined variable " << var << "\033[0m"
                << std::endl;
      return "";
    }
  }
...
private:
  ...
  std::string caret_operator(std::string str_arg) {
    ...
    if (str_arg.find('^') != std::string::npos) {
      ...
      int r{
          std::stoi(str_arg.substr(str_arg.find('^') + 1, std::string::npos))};

      if (l[0] == '$') {
        l = sym_table.access(l.substr(1));
      }
      ...
      if (str_arg.at(0) == '$') {
        return sym_table.access(str_arg.substr(1));
      } else {
        return str_arg;
      }
    }
  ...
  void readLine() {
    ...
    std::cout << l_value << " is set to " << r_value << std::endl;
  }
...
public:
  Variable(const std::string &_var_name, const std::string &_expr)
      : var_name{_var_name}, var_value{} {};
};
